...
if(isset($_POST) && !empty($_POST)){
    if (empty($_POST["1"]) || empty($_POST["2"]) || empty($_POST["3"]) || empty($_POST["4"]) || empty($_POST["5"])) {
        ?> 
           ...
           Nenhum campo resposta não pode estar vazio...
         </div>
       <?php
...
<div class="card mx-auto justify-content-center" style="width: 50%; height:600px; margin: 50px 0 50px 0">
...
<?php
        while($linha = mysqli_fetch_array($resultadoAlternativas)){
            ...
            if($linha["correta"]){
                ?>
                    <div class="alert alert-success"> Alternativa correta: <?php echo"".$linha['alternativa']."<br>".""; ?> </div>
                <?php  
        }      else {
